...
export const createServerClient = () => {
  ...
  if (!supabaseUrl || !serverKey) {
    console.warn("Server Supabase credentials not configured. Using demo mode.")
    return null
  }
...
export const testSupabaseConnection = async (): Promise<boolean> => {
  ...
  try {
    ...
    const testPromise = supabase.from("products").select("count", { count: "exact", head: true })
...
export const checkTablesExist = async (): Promise<boolean> => {
  ...
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Table check timeout")), 3000))

    const testPromise = supabase.from("products").select("id", { head: true }).limit(1)
