...
class NewsletterService {
  ...
  addSubscriber(email: string, source = "homepage"): string {
    ...
    const existingSubscriber = this.subscribers.find((s) => s.email.toLowerCase() === email.toLowerCase())
  ...
  getStats() {
    ...
    return {
      ...
      unsubscribed,
      todaySubscribers,
      conversionRate: active > 0 ? ((active / (active + unsubscribed)) * 100).toFixed(1) : "0",
    }
  }
  ...
  exportToCSV(): string {
    ...
    const csvContent = [
      ...
      ...this.subscribers.map((sub) =>
        [sub.email, sub.status, new Date(sub.subscribed_at).toLocaleDateString(), sub.source].join(","),
      ),
    ].join("\n")
  ...
  subscribe(listener: (subscribers: NewsletterSubscriber[]) => void): () => void {
