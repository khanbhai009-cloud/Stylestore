...
export default function ProductCard({ product, onProductClick, viewMode = "grid" }: ProductCardProps) {
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="relative md:w-48 h-48 md:h-auto overflow-hidden bg-gray-100">
            ...
            {discountPercentage > 0 && (
              <div className="absolute top-2 left-2">
                <Badge className="bg-red-600 text-white font-bold">-{discountPercentage}%</Badge>
              </div>
            )}
            ...
            <div className="absolute top-2 right-2 flex flex-col gap-1">
              {product.tags.slice(0, 2).map((tag, index) => (
                <Badge key={index} variant="secondary" className="text-xs font-semibold bg-white/90 text-gray-800">
                  {tag}
                </Badge>
              ))}
            </div>
          ...
          <div className="flex-1 p-6">
            ...
            <div className="flex gap-3">
              <Button onClick={handleBuyNowClick} className="flex-1 bg-primary hover:bg-primary/90">
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-orange-200 dark:border-orange-700 overflow-hidden hover:shadow-xl hover:shadow-orange-500/20 transition-all duration-300 transform hover:-translate-y-1 group neon-glow">
      ...
      <div className="relative aspect-square overflow-hidden bg-gray-100">
        {imageError ? (
          ...
          <>
            ...
            <img
              src={product.image_url || "/placeholder.svg?height=400&width=400&text=Product"}
              alt={product.name}
      ...
      <div className="p-4 space-y-3">
        <h3 className="font-semibold text-lg text-gray-800 dark:text-white line-clamp-2">{product.name}</h3>
          ...
          <div className="flex items-center">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                className={`h-4 w-4 ${
                  i < Math.floor(averageRating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300 dark:text-gray-600"
                }`}
              />
            ))}
          </div>
          <span className="text-sm text-gray-600 dark:text-gray-400">({averageRating.toFixed(1)})</span>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          {product.original_price && (
            <span className="text-sm text-gray-500 dark:text-gray-500 line-through">${product.original_price}</span>
          )}
        </div>
        ...
        {/* Social Proof */}
        <div className="text-xs text-gray-500 dark:text-gray-500 text-center">{product.clicks} people viewed this</div>
      </div>
    </div>
  )
}
