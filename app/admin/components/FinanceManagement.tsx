...
import { DollarSign, TrendingUp, CreditCard, PieChart, Calendar, Target, Activity } from "lucide-react"
          ...
          <CardContent className="p-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">${financeData.totalRevenue.toFixed(2)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">${financeData.monthlyRevenue.toFixed(2)}</p>
                <div className="flex items-center space-x-1 mt-1">
                  <TrendingUp className="h-3 w-3 text-green-600" />
                  <span className="text-xs text-green-600">+{financeData.revenueGrowth}%</span>
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">${financeData.averageOrderValue.toFixed(2)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              {["week", "month", "year"].map((period) => (
                <button
                  ...
                  className={`px-3 py-1 text-sm rounded-md capitalize ${
                    selectedPeriod === period ? "bg-red-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                  }`}
                >
                  {period}
                </button>
            ...
            {financeData.topProducts.map((product, index) => (
              <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm text-gray-600">${(product.revenue / product.orders).toFixed(2)} avg</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold">${(financeData.totalRevenue * 0.85).toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Shipping Fees</span>
                <span className="font-semibold">${(financeData.totalRevenue * 0.1).toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Tax Collected</span>
                <span className="font-semibold">${(financeData.totalRevenue * 0.05).toFixed(2)}</span>
              </div>
              <div className="border-t pt-2">
                <div className="flex justify-between items-center font-bold">
                  <span>Total Revenue</span>
                  <span className="text-green-600">${financeData.totalRevenue.toFixed(2)}</span>
                </div>
              </div>
            </div>
          </CardContent>
